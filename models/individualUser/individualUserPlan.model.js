...
const IndividualUserPlanSchema = new mongoose.Schema(
  {
    individualUserId: {
      type: mongoose.Schema.Types.ObjectId,
      ref: "IndividualUser",
      required: [true, "IndividualUser Id is required !"],
    },
    name: {
      type: String,
      trim: true,
...
IndividualUserPlanSchema.pre("save", function (next) {
  if (
    this.endDate &&
    this.billingCycle &&
...
IndividualUserPlanSchema.virtual("remainingDays").get(function () {
  return Math.ceil((this.endDate - new Date()) / (1000 * 60 * 60 * 24));
});
// Virtual property to get usage percentage
IndividualUserPlanSchema.virtual("usagePercentage").get(function () {
...
module.exports = mongoose.model("IndividualUserPlan", IndividualUserPlanSchema);
