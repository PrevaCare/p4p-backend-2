...
} = require("../../helper/messageScheduler/appointmentMessage.helper");
...
const sendAppointmentNotification = async (req, res) => {
  ...
  const appointments = await patientAppointmentModel
    .find({
  ...
  for (const appt of appointments) {
    ...
    res
      .status(200)
      .json({ message: "Appointment notification sent successfully" });
  }
};
