...
const getDashboardData = async (req, res) => {
  try {
    ...
    const activeCorporateUsers = await User.find({
      role: "Employee",
    }).countDocuments();
    const activeIndividualUsers = await User.find({
      role: "IndividualUser",
    }).countDocuments();
...
const getRecentAddedUsers = async (req, res) => {
  try {
    const user = await User.find({
      role: { $in: ["Employee", "IndividualUser"] },
    })
      .sort({ createdAt: -1 })
      .limit(4)
    ...
    const formattedCorporate = corporate.map((user) => {
      const timeAgo = formatDistanceToNow(new Date(user.updatedAt), {
        addSuffix: true,
      });
      return {
    ...
    const globalPackages = await GlobalPlan.find({})
      .sort({ updatedAt: -1 })
      .limit(4)
